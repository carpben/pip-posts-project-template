...
class TestAPI(unittest.TestCase):
    ...
    def test_delete_non_existend_post(self):
        """ Deleting a post that does not exist """

        response = self.client.delete("/api/posts/5",
        headers=[("Accept", "application/json")]
        )
        ...
        data = json.loads(response.data.decode("ascii"))
        self.assertEqual(data["message"], "Could not find post with id 5")
    ...
    def test_get_posts_title_body(self):
        """ Filtering posts by title """
        postA = models.Post(title="Post with bells", body="body with bells")
        postB = models.Post(title="Post with whistles", body="empty body")
        postC = models.Post(title="Post with bells and whistles",
                            body="body with bells")
